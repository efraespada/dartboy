...
class PPU {
  ...
  List<Palette?> bgPalettes = List.generate(8, (index) => null);

  /// Sprite palettes. 0-7 used on CGB, 0-1 used on GB.
  List<Palette?> spritePalettes = List.generate(8, (index) => null);
  ...
  void reset() {
    ...
    this.bgPalettes = List.generate(8, (index) => null);
    this.spritePalettes = List.generate(8, (index) => null);
    this.gbcBackgroundPaletteMemory = new List.generate(0x40, (index) => 0);

    this.buffer =
        new List.generate(PPU.LCD_WIDTH * PPU.LCD_HEIGHT, (index) => 0);
    ...
    this.gbcSpritePaletteMemory = new List.generate(0x40, (index) => 0);
    this
        .gbcSpritePaletteMemory
        .fillRange(0, this.gbcSpritePaletteMemory.length, 0);

    this.spritesDrawnPerLine = new List.generate(PPU.LCD_HEIGHT, (index) => 0);
    ...
    if (this.cpu.cartridge.gameboyType == GameboyType.COLOR) {
      ...
      for (int i = 0; i < this.spritePalettes.length; i++) {
        List<int> colors = new List.generate(4, (index) => 0);
      ...
      this.bgPalettes[0] =
          new GBPalette(this.cpu, colors.bg, MemoryRegisters.BGP);
      this.spritePalettes[0] =
          new GBPalette(this.cpu, colors.obj0, MemoryRegisters.OBP0);
      this.spritePalettes[1] =
          new GBPalette(this.cpu, colors.obj1, MemoryRegisters.OBP1);
    }
  }
  ...
  void loadPalettesFromMemory(List<int> from, List<Palette?> to) {
    ...
    for (int i = 0; i < 8; i++) {
      // 4 ints per palette
      for (int j = 0; j < 4; ++j) {
        if (to[i] != null) updatePalette(from, to[i]!, i, j);
      }
    }
  }
  ...
  void drawBackgroundTiles(List<int> data, int scanline) {
    ...
    for (int x = 0; x < 21; x++) {
      ...
      if (this.bgPalettes[gbcPalette] == null) {
        return;
      }
  ...
  void drawWindow(List<int> data, int scanline) {
    ...
    for (int x = this.getWindowPosX() ~/ 8; x < 21; x++) {
      ...
      if (this.bgPalettes[gbcPalette] == null) return;
      this.drawTile(
          this.bgPalettes[gbcPalette]!,
          data,
          posX + x * 8,
