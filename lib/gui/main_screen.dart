    ...
    if (!MainScreen.keyboardHandlerCreated) {
      ...
      RawKeyboard.instance.addListener((RawKeyEvent key) {
        ...
        if (key is RawKeyUpEvent) {
          MainScreen.emulator.buttonUp(keyMapping[keyCode] ?? 0);
        } else if (key is RawKeyDownEvent) {
          MainScreen.emulator.buttonDown(keyMapping[keyCode] ?? 0);
        }
      });
    }
                        ...
                        new ElevatedButton(
                            onPressed: () {
                              if (MainScreen.emulator.state !=
                                ...
                                Modal.alert(context, 'Error',
                                    'There is a ROM already loaded. Reset before loading new ROM.');
                                return;
                              }
